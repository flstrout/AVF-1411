...
//Rare exceptions to indicate a bug in the _stroutfred_avf1411_project3 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_stroutfred_avf1411_project3 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_stroutfred_avf1411_project3]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_avf1411_project3.module"
