...
NSString * StroutFred_AVF1411_Project3$ModuleRequireFormat = @"(function(exports){"
...
@implementation StroutFred_AVF1411_Project3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_stroutfred_avf1411_project3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_stroutfred_avf1411_project3 gc];
}
...
{
	// create StroutFred_AVF1411_Project3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "StroutFred_AVF1411_Project3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_stroutfred_avf1411_project3 = [[StroutFred_AVF1411_Project3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_stroutfred_avf1411_project3];
	
	NSString *_stroutfred_avf1411_project3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _stroutfred_avf1411_project3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_stroutfred_avf1411_project3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:StroutFred_AVF1411_Project3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_avf1411_project3.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.stroutfred_avf1411_project3.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
