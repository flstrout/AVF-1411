...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kStroutFred_AVF1411_Project3UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kStroutFred_AVF1411_Project3UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a StroutFred_AVF1411_Project3 app using the node.js CLI
