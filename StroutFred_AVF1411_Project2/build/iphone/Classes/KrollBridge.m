...
NSString * StroutFred_AVF1411_Project2$ModuleRequireFormat = @"(function(exports){"
...
@implementation StroutFred_AVF1411_Project2Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_stroutfred_avf1411_project2);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_stroutfred_avf1411_project2 gc];
}
...
{
	// create StroutFred_AVF1411_Project2 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "StroutFred_AVF1411_Project2" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_stroutfred_avf1411_project2 = [[StroutFred_AVF1411_Project2Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_stroutfred_avf1411_project2];
	
	NSString *_stroutfred_avf1411_project2NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _stroutfred_avf1411_project2NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_stroutfred_avf1411_project2 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:StroutFred_AVF1411_Project2$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_avf1411_project2.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.stroutfred_avf1411_project2.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
